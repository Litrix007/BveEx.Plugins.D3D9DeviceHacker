...
namespace BveEx.Plugins.D3D9DeviceHacker
{
    ...
    public static class PluginConfig
    {
        ...
        {
            ConfigPath = Path.Combine(baseDirectory, "D3D9DeviceHacker.ini");
            ...
            try
            {
                foreach (var rawLine in File.ReadAllLines(ConfigPath))
                {
                    ...
                    switch (key)
                    {
                        ...
                        case "MaxFrameLatency": MaxFrameLatency = ParseInt(val, MaxFrameLatencyDefault, 1, 16); break;
                        case "EnableDebug": EnableDebug = ParseBool(val, EnableDebugDefault); break;
        ...
        private static T ParseEnum<T>(string s, T def) where T : struct =>
            Enum.TryParse(s, true, out T v) ? v : def;
